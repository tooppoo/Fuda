...
| `question_comment_id` | Fuda が投稿した clarification question comment の GitHub comment id。integer, minimum `1` |
...
| `run_state` | 意味 |
|---|---|
| `succeeded` | Fuda Run が正常終了し、必要な終了処理が完了した。GitHub source issue の close 状態を意味しない |
| `aborted` | ユーザー操作によって中断された |
| `failed` | 不正な状態・agent エラー・git エラーなどにより失敗した |
