...
| reviewer JSONが不正 | なし | `run_state = failed` + `last_error.code = invalid_reviewer_output` で停止する |
...
Kogoto は `issue-state.json` の `issue_workflow_state` を参照して判断する。

| `issue_workflow_state` | 扱い |
|---------|------|
| `active` | `resume` 候補として表示する |
| `waiting_for_human` | 回答待ちとして表示する |
| `pr_created` | `close` を促す |
| `completed` | 完了済みとして表示する。`close` は促さない |
| `aborted` / `failed` | 中断・失敗済みとして表示する |
| 状態不明（ファイルなし・parse失敗） | 停止し、人間確認を求める |
