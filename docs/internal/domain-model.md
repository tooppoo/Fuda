...
Fuda は、GitHub Issue を一枚の「作業札」とみなし、writer agent・reviewer agent・人間の確認を往復させながらIssue解決を支援するrunnerである。

Fudaは自律開発ツールではない。Issueを単位として小さく作業を進め、必要な箇所で人間が確認・回答・停止できるようにすることを目的とする。
...
Issue を一枚の「作業札」とみなし、その札を処理するrunner。

CLI コマンド名は `fuda`。

### Run

`fuda resolve <issue-number>` によって開始される一連の作業単位。
...
flowchart TD
    ...
    DECISION -- human_review_required / invalid_review_output --> STOP
...
Fudaはこれを検出したら、Issueに質問コメントを投稿して停止する。

### 回答とみなすもの

* Fudaの質問コメント投稿後に追加されたIssueコメント
* Fuda自身の投稿ではないコメント
* bot投稿ではないコメント
* `fuda answer` で投稿されたコメント
...
| reviewer出力が不正 | `invalid_review_output` | 停止する |
...
上位層（`app`, `runner`）は抽象（`tracker`）にのみ依存し、実装（`tracker/github`）を直接参照しない。具体的な実装への依存は `app` または `cmd/fuda` でインジェクションする。
