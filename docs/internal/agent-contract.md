...
`blocked_by_ambiguity` が返されたにもかかわらず `blocked.questions` に `blocking: true` のものが1件もない場合、runner は `invalid_writer_output` として扱う（`run_state = failed` + `last_error.code = invalid_writer_output`）。
