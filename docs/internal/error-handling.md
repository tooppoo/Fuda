...
- `retry_count` は writer への修正依頼回数を表す。初期値は `0`。`testing` → `fixing` 遷移を `run.json` に永続化するタイミングで加算する（writer 呼び出しより前）。
- write / fix サイクルが新しく始まるたびに `retry_count` を `0` にリセットする。
- `review_loop.completed_review_rounds` とは独立して管理する。

### v0 での retry 上限

v0 では `retry_count` の上限を **2** に固定する。この値は設定ファイルに公開しない。

### 失敗時の挙動

- `retry_count < 2` の場合: `retry_count` を加算して `run_state = fixing` に遷移し（この永続化を完了させてから）、writer に修正を依頼する。
- `retry_count >= 2` でさらに失敗した場合: PR は作成せず `run_state = failed` で停止する。
