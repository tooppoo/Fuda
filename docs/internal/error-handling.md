# Error Handling

## Scope

このドキュメントは Kogoto Run における失敗時の挙動を定義する。

正常系フローは `processing-sequence.md` を参照。`run_state` の定義と状態遷移は `state-machine.md` を参照。

---

## 基本方針

- `run_state` は Run ライフサイクル状態のみを表す。失敗原因の詳細は `last_error.code` で表す。
- Kogoto が安全に判断できない場合は、推測で継続せず停止する。
- 既存 branch / worktree / run artifact は自動上書きしない。
- agent raw output は debugging artifact として保存する。parse / normalization 失敗時も削除しない。
- GitHub Issue / PR / branch / worktree への破壊的操作は、人間の明示判断なしに行わない。

---

## `invalid_agent_output` の扱い

`invalid_agent_output` を `run_state` としては使用しない。`run_state` はライフサイクル状態を表すフィールドであり、`invalid_agent_output` は失敗原因である。

agent 出力不正は、`run_state = failed` と `last_error.code` の組み合わせで表す。

- writer 出力不正: `last_error.code = invalid_writer_output`
- reviewer 出力不正: `last_error.code = invalid_reviewer_output`

```json
{
  "run_state": "failed",
  "last_error": {
    "code": "invalid_reviewer_output",
    "phase": "reviewing",
    "message": "Reviewer output could not be parsed or normalized.",
    "recoverability": "retryable_after_human_confirmation",
    "artifact": "review-1.raw.txt",
    "occurred_at": "2026-06-07T00:00:00Z"
  }
}
```

`last_error` の永続形式は `schemas/run.schema.json` を正とする。error code の意味・復旧方針は本ドキュメントの Error Matrix を正とする。

---

## Verification Failure Policy

test / lint / typecheck 失敗は review loop とは別の **verification failure** として扱う。

### `verification_loop` ストレージ

verification retry の進捗は `run.json` の `verification_loop` オブジェクトに必ず記録する。

```json
{
  "verification_loop": {
    "max_retries": 2,
    "used_retries": 0
  }
}
```

- verification が開始された時点で `verification_loop` を `run.json` に書く。
- `used_retries` は writer への修正依頼回数を表す。初期値は `0`。`testing` → `fixing` 遷移を `run.json` に永続化するタイミングで加算する（writer 呼び出しより前）。
- `used_retries` は run 全体での累積カウントであり、review round をまたいでリセットしない。
- `review_loop.completed_review_rounds` とは独立して管理する。両者を混同してはならない。

### retry 上限

retry 上限は `config.toml` の `[verification] max_retries` で設定する（初期値: `2`、最小値: `0`）。実行時に採用した値は `verification_loop.max_retries` として `run.json` に記録する。

### 失敗時の挙動

- `used_retries < max_retries` の場合: `used_retries` を加算して `run_state = fixing` に遷移し（この永続化を完了させてから）、writer に修正を依頼する。
- `used_retries >= max_retries` でさらに失敗した場合: PR は作成せず `run_state = failed` で停止する。

停止時には、失敗した command、exit code、要約 log、次に取れる操作を表示する。

### ログ保持方針

- verification 失敗時の stdout/stderr 全文は run artifact として別ファイルに保持する。
- `run-summary.json` には stdout/stderr 全文を含めない。`failure_summary.artifact_ref` で参照する。

---

## Raw Output Preservation Policy

- agent の raw output は、成否にかかわらず artifact として保存する。
- parse / schema validation / normalization に失敗した場合、正規化済み JSON は書かない。raw output のみ保存する。
  - writer plan output 失敗: `plan.raw.txt` のみ保存、`plan.json` は書かない
  - writer write / fix output 失敗: `implement.log` または `fix-N.log` を保存し、追加の正規化済み JSON は書かない
  - reviewer output 失敗: `review-N.raw.txt` のみ保存、`review-N.json` は書かない
- raw output は `run-summary.json` に含めない。

---

## Existing Branch / Worktree Conflict Policy

既存 branch / worktree / path に conflict がある場合、Kogoto は自動上書きしない。

**Kogoto-managed run を特定できる場合**: 新しい run は作らず、既存 run の状態を表示して停止する。`run_state` は既存 run のものを維持する。

**Kogoto-managed run を特定できない場合**: 新しい run を `run_state = failed` として作成して停止する。

---

## Error Matrix

Run の各 phase における失敗ケースを以下に定義する。`Run state` 列は失敗時の最終 `run_state` を示す。`Recoverability` 列は `last_error.recoverability` の値を示す（enum の意味と resume 判断は後述「Recoverability と resume」を正とする）。

| Case | Phase | Error code | Recoverability | Behavior | Run state |
|---|---|---|---|---|---|
| GitHub認証失敗 | `loading_issue` | `github_auth_failed` | `retryable_after_manual_fix` | 認証設定を直して再実行するまで停止 | `failed` |
| Issueが存在しない | `loading_issue` | `issue_not_found` | `terminal` | worktree を作らず停止。再対応は新規 run | `failed` |
| Issueがclosed | `loading_issue` | `issue_closed` | `retryable_after_human_confirmation` | closed Issue を勝手に再 open せず停止 | `failed` |
| Issue番号がPRを指している | `loading_issue` | `issue_is_pr` | `terminal` | PR を Issue として扱わず停止。state ファイル未作成のため cleanup 不要 | `failed` |
| main更新失敗 | `preparing_worktree` | `main_update_failed` | `retryable` | worktree 作成前に停止 | `failed` |
| worktree作成失敗 | `preparing_worktree` | `worktree_create_failed` | `retryable_after_manual_fix` | 既存ファイルを壊さず停止 | `failed` |
| branch既存 (Kogoto管理runあり) | `preparing_worktree` | `branch_already_exists` | —（既存 run を resume） | 既存 run の状態を表示して停止 | 既存 run の `run_state` |
| branch既存 (Kogoto管理run不明) | `preparing_worktree` | `branch_already_exists` | `manual_inspection_required` | 上書きせず停止 | `failed` |
| worktree path既存 (Kogoto管理runあり) | `preparing_worktree` | `worktree_path_already_exists` | —（既存 run を resume） | 既存 run の状態を表示して停止 | 既存 run の `run_state` |
| worktree path既存 (Kogoto管理run不明) | `preparing_worktree` | `worktree_path_already_exists` | `manual_inspection_required` | 上書きせず停止 | `failed` |
| writer agent起動失敗 | `planning` / `writing` | `writer_launch_failed` | `retryable` | raw なしで停止 | `failed` |
| writer出力が不正 | `planning` / `writing` / `fixing` | `invalid_writer_output` | `retryable_after_human_confirmation` | raw 保存、正規化 JSON を書かず停止 | `failed` |
| blocked コメント投稿失敗 | `posting_comment` | `runner_error` | `retryable` | issue-state / run.json を failed に更新して停止 | `failed` |
| test / lint / typecheck失敗 | `testing` | `verification_failed` | `manual_inspection_required` | retry 中は `testing` → `fixing` を経由。上限到達後: PR 作成せず停止、失敗 command / exit code / 要約 log を表示 | `failed` |
| commit対象変更なし | `committing` | `nothing_to_commit` | `manual_inspection_required` | PR 作成せず停止 | `failed` |
| commit失敗 | `committing` | `commit_failed` | `retryable_after_manual_fix` | diff を保持して停止 | `failed` |
| reviewer agent起動失敗 | `reviewing` | `reviewer_launch_failed` | `retryable` | review round を進めず停止 | `failed` |
| reviewer出力が不正 | `reviewing` | `invalid_reviewer_output` | `retryable_after_human_confirmation` | `review-N.raw.txt` のみ保存、`review-N.json` を書かず停止 | `failed` |
| push失敗 | `pushing` | `push_failed` | `retryable` | local commit を保持して停止 | `failed` |
| PR作成失敗 | `creating_pr` | `pr_create_failed` | `retryable` | branch / commit を保持して停止 | `failed` |

`recoverability` の per-occurrence の扱いと `last_error.code` 別の default 対応表は `schemas/run.schema.md` の「`recoverability` enum」を正とする。

---

## Recoverability と resume

`last_error.recoverability` は表示専用情報ではなく、`resume` / retry / abort / manual inspection の判断に使う制御情報である。enum 値は次の意味を持つ。

| `recoverability` | 自動再開 | 区分 |
|---|:---:|---|
| `retryable` | する | resume 可能（自動再試行） |
| `retryable_after_human_confirmation` | しない | resume 可能（人間の確認が前提） |
| `retryable_after_manual_fix` | しない | resume 可能（外部状態の手動修正が前提） |
| `manual_inspection_required` | しない | resume 不可（調査が前提） |
| `terminal` | しない | resume 不可（この Run では復旧不能） |

判断ルール:

- `recoverability = retryable` の場合のみ、`kogoto resume` は失敗 phase から自動的に再試行してよい。
- `recoverability = retryable_after_human_confirmation` / `retryable_after_manual_fix` の場合、Kogoto は自動再開しない。人間の確認または外部状態の修正を経た明示的な `kogoto resume` 実行でのみ再開する。
- `recoverability = manual_inspection_required` の場合、Kogoto は自動再開してはならない。人間が artifact / 状態を調査するまで再開を保留する。
- `recoverability = terminal` の場合、`kogoto resume` は再開を拒否する。再対応は新規 run として扱う。

`run.json` 自体が parse / validation error で壊れている場合は、`recoverability` の値によらず resume してはならない（`run.schema.md` の Recovery policy を参照）。

### CLI が次に取れる操作を示す方針

Run が `failed` で停止した場合、CLI は失敗内容に加えて `recoverability` に応じた「次に取れる操作」を提示する。

| `recoverability` | CLI が提示する次の操作 |
|---|---|
| `retryable` | `kogoto resume <issue>` で再試行できる旨を表示する |
| `retryable_after_human_confirmation` | 状態を確認のうえ `kogoto resume <issue>` する旨を表示する |
| `retryable_after_manual_fix` | 修正対象（設定 / git / ファイル等）を直してから `kogoto resume <issue>` する旨を表示する |
| `manual_inspection_required` | 関連 artifact / log の調査を促し、必要なら `kogoto abort <issue>` を案内する |
| `terminal` | resume 不可である旨と、`kogoto abort <issue>` または新規対応を案内する |

verification 失敗（retry 上限到達）のように停止時に詳細を持つケースでは、失敗した command / exit code / 要約 log とあわせて上記の次操作を表示する。

---

## Planning / Comment 投稿失敗時のクリーンアップ

`planning` フェーズおよび blocked コメント投稿（`posting_comment`）フェーズで失敗した場合、state ファイルが既に作成済みのため次のクリーンアップを行う。

1. `run.json.run_state` を `failed` にする
2. `run.json.last_error` に失敗詳細（`code` / `phase` / `message`）を書く
3. `issue-state.json.issue_workflow_state` を `failed` にする
4. `issue-state.json.runs` の対応 RunRecord の `run_result` を `failed` にする

クリーンアップ自体が失敗した場合、元の失敗 error と cleanup error を連結して返す（`errors.Join`）。クリーンアップ失敗が元の失敗原因を隠蔽しないようにする。

`loading_issue` フェーズ中（`issue-state.json` / `run.json` 未作成）の失敗は state ファイルを作成しないため、このクリーンアップは不要。

---

## `run-summary.json` への失敗原因記録

Run が `failed` で終端した場合、`run-summary.json` に失敗概要を `failure_summary` として記録する。

`failure_summary` は `run.json.last_error` から summary 用に安全な情報のみを転記する。

| `run.json.last_error` | `run-summary.json.failure_summary` |
|---|---|
| `code` | `code` |
| `phase` | `phase` |
| `message` | `message` |
| `recoverability` | `recoverability` |
| `occurred_at` | `occurred_at` |
| `artifact` | `artifact_ref` |

`run-summary.json` には以下を含めない。

- 巨大ログ（詳細 test log など）
- 詳細 diff
- 機密情報（token、credential など）
- agent prompt 全文
- command output 全文

詳細 log は run artifact として別ファイルに保持し、`failure_summary.artifact_ref` で参照する。`run-summary.schema.md` を参照。

`aborted` は失敗ではなく user / policy による中断であるため、`failure_summary` は持たせない。

---

## 既存仕様との接続

| 仕様 | 接続点 |
|---|---|
| `state-machine.md` | `run_state` enum および状態遷移 trigger |
| `processing-sequence.md` | resolve / close / blocked フローと失敗箇所の対応 |
| `schemas/run.schema.json` | `run_state` enum と `last_error` の定義 |
| `schemas/run-summary.schema.md` | `terminal_state = failed` と `completion_result` の対応 |
| `schemas/README.md` | raw output 保存方針と schema validation / error classification policy |
